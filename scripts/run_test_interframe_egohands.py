...
def to_npy(detections):
    ...
    # Helper function to ensure NumPy conversion
    def to_numpy(x):
        return x.cpu().numpy() if isinstance(x, torch.Tensor) else np.array(x)

    # Convert data safely
    data['t'] = to_numpy(detections['t'])
    data['x'] = to_numpy(detections['boxes'][:, 0])
    data['y'] = to_numpy(detections['boxes'][:, 1])
    data['w'] = to_numpy(detections['boxes'][:, 2]) - data['x']
    data['h'] = to_numpy(detections['boxes'][:, 3]) - data['y']
    data['class_id'] = to_numpy(detections['labels'])
    data['class_confidence'] = to_numpy(detections['scores'])
...
if __name__ == '__main__':
    ...
    print("init datasets")
    root_test = f"/cfs/earth/scratch/kotabha1/temp"
