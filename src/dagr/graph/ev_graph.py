...
class AsyncGraph:
    ...
    def forward(self, batch, pos, collect_edges=True):
        ...
        # print("edge_buffer:", self.edge_buffer.shape, self.edge_buffer.dtype, self.edge_buffer.device)
        assert self.edge_buffer is not None, "edge_buffer is not initialized"
        assert self.edge_buffer.is_cuda, "edge_buffer is not on CUDA"
        assert self.edge_buffer.numel() > 0, "edge_buffer is empty"
